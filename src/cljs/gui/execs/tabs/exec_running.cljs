...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
(ns gui.execs.tabs.exec_running
...
(defn- show-all ""
  []
  (logs/info "Showing ALL running executions...")
  ...
  (when-not @EXECUTIONS_LOADED
    (do
      ...
      (re-frame/dispatch [::events/set-exec-running (doall (for [x @VARS/ALDE_EXECS_RUNNING] (draw-row x)))])
      ))
...
(defn- show-by-app ""
  [id-configs]
  (logs/info "Showing running executions by app [" @COMPLETED_APP_ID "] (ids: " (str id-configs) ") ...")
  (when-not @EXECUTIONS_LOADED
    (do
      (reset! TOTAL_SHOWN 0)
      (reset! VARS/TAB_LOADING true)
      (let [filtered-execs (filter #(contains? id-configs (% :execution_configuration_id)) @VARS/ALDE_EXECS_RUNNING)]
        (re-frame/dispatch [::events/set-exec-running (doall (for [x filtered-execs] (draw-row x)))]))))
  (reset! VARS/TAB_LOADING false)
  (reset! EXECUTIONS_LOADED true))
...
(defn- show-by-app-conf ""
  [id-config]
  (logs/info "Showing running executions by app [" @COMPLETED_APP_ID "] configuration [" (str id-config) "] ...")
  (when-not @EXECUTIONS_LOADED
    (do
      (reset! TOTAL_SHOWN 0)
      (reset! VARS/TAB_LOADING true)
      (let [filtered-execs (filter #(= id-config (% :execution_configuration_id)) @VARS/ALDE_EXECS_RUNNING)]
        (re-frame/dispatch [::events/set-exec-running (doall (for [x filtered-execs] (draw-row x)))]))))
  (reset! VARS/TAB_LOADING false)
  (reset! EXECUTIONS_LOADED true))
...
(defn panel-component []
  (let [total-execs           (re-frame/subscribe [::subs/total-execs])
        total-execs-running   (re-frame/subscribe [::subs/total-execs-running])
        t-execs               (re-frame/subscribe [::subs/exec-running])]
    [:div
      [:div.row
        [:div.col-sm-3
          [:h5
        ...
        [:div.col-sm-3
          [:h5
            [:span.badge.badge-pill.badge-info "Running: "]
            [:span.badge.badge-pill.badge-secondary @total-execs-running]]]
        [:div.col-sm-4 ]
        [:div.col-sm-2
          [:button.badge.badge-pill.btn-sm {:title "Reload executions"
            :on-click   #(do
                          (logs/info "Refreshing running executions...")
                          (restapi/get-executions-running VARS/update-execs-running)
                          (reset! EXECUTIONS_LOADED false)
                          (show-all))} "refresh"]]]
      ...
      [:div.row {:style {:margin-top "-5px"}}
        [:div.col-sm-8
          ...
          [:span.badge.badge-pill.badge-light {:style {:color "lightgray"}}
            [:i "Running executions with no information about the execution configuration are not showed"]]]]
  ...
  (r/create-class
    {;; Mounting
     :component-did-mount     #(do
                                (reset! EXECUTIONS_LOADED false)
                                (show-all)
                                (logs/info "EXEC-RUNNING: component-did-mount"))
     ;; Unmounting
     :component-will-unmount  #(do
                                (logs/info "EXEC-RUNNING: component-will-unmount")
                                (re-frame/dispatch [::events/set-exec-running [:tr]]))
     ;; render
