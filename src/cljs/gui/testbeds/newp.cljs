        ...
        [:div.row
          [:div.col-sm-12
            [:h5 {:style {:margin-top "-25px" :text-align "left"}}
              [:span.badge.badge-pill.badge-success "Create new node"]]]]
        ;; content
          ...
          [:div.col-sm-3
            [:h5 {:style {:margin-top "5px" :text-align "left"}}
              ...
              [:button.badge.badge-pill.btn-sm.btn-danger {:title "Cancel operation and close panel"
                :data-toggle "collapse" :data-target "#collapseNew2" :aria-expanded "false" :aria-controls "collapseNew2"} "cancel"]]]]]]])
